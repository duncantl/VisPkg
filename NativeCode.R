...
function(filename= NA) {
    ...
    update = function(cur, parent) {
        ...
        types[[ length(types) + 1L ]] <<- if(isCRoutineDef(cur, kind)) "FunctionDefn" else mapCursorTypes(kind)
    ...
    list(update = update, result = function() data.frame(value = rep(1, length(cursors)), type = types, #mapCursorTypes(cursors),
                                                         file = files, name = names, lineNumber = lines, kind = cursors))
}

isCRoutineDef =
function(cur, kind = getCursorKind(cur))
{
    if(kind == CXCursor_FunctionDecl) {
        k = children(cur)
        if(length(k) > 0 && getCursorKind(k[[length(k)]]) == CXCursor_CompoundStmt)
            return(TRUE)
    }
...
mapCursorTypes =
function(vals)
    gsub("^CXCursor_", "", names(CXCursorKind)[match(vals, CXCursorKind)])
...
NativeCodeTypeMap = c("Constructor" = "green", EnumDecl = "gold", "FunctionDefn" = "red", "FunctionDecl" = "grey", "StructDecl" = "blue", "TypedefDecl" = "lightblue", "VarDecl" = "brown")
...
if(FALSE) {
    ...
    xml.cfiles = xpdFileNames("~/GitWorkingArea/XML/src", pattern = "\\.c")
    xml.nativeInfo = lapply(xml.cfiles, getToplevelNativeInfo, includes = xml.includes, args = xml.args)
    names(xml.nativeInfo) = xml.cfiles
    origShowFiles(vals = xml.nativeInfo, colorMap = NativeCodeTypeMap)
}
...
if(FALSE)
{
    ...
    origShowFiles(vals = z2, colorMap = NativeCodeTypeMap)    
